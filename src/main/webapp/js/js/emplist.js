...
Ext.onDocumentReady(function () {
  ...
  var store = new Ext.data.Store({
    ...
    proxy: {
      ...
      // 	read: 'POST'
      // },
      url: "http://localhost:8080/SpringFinal/SQL/fetch.action",
      reader: {
        type: "json",
        rootProperty: "films",
        totalProperty: "total",
          ...
          "-",
          {
            ...
            listeners: {
              click: function () {
                ...
                console.log(
                  editStore.getData().items[selectedrows.length - 1].data.title
                    .film_id
                );
                Ext.getCmp("director").setValue(
                  selectedrows[selectedrows.length - 1][0].director
                );
                Ext.getCmp("des").setValue(
                  selectedrows[selectedrows.length - 1][0].description
                );
                Ext.getCmp("title").setValue(
                  selectedrows[selectedrows.length - 1][0].title
                );
                Ext.getCmp("rYear").setValue(
                  selectedrows[selectedrows.length - 1][0].release
                );
                Ext.getCmp("sfeatures").setValue(
                  selectedrows[selectedrows.length - 1][0].features
                );
                Ext.getCmp("rating").setValue(
                  selectedrows[selectedrows.length - 1][0].rating
                );
                Ext.getCmp("language").setValue(
                  selectedrows[selectedrows.length - 1][0].language
                );
              },
            },
            //handler: {
...
win = Ext.create("Ext.window.Window", {
  ...
  items: {
    xtype: "form",
    id: "myForm",
    url: "http://localhost:8080/SpringFinal/SQL/add.action",
...
let Edi = Ext.create("Ext.window.Window", {
  ...
  items: {
    xtype: "form",
    id: "Form",
    url: "http://localhost:8080/SpringFinal/SQL/edit.action",
    border: false,
    closeAction: "hide",
    plain: true,
    layout: "vbox",
  ...
  buttons: [
    {
      ...
      handler: function () {
        Ext.Ajax.request({
          url: "http://localhost:8080/SpringFinal/SQL/delete.action",
          method: "POST",
          // timeout: 60000,
          jsonData: {
            film_id: deleteData,
